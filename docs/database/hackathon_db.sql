...
CREATE OR REPLACE FUNCTION check_organizzatore_role()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
...
CREATE TRIGGER trigger_check_organizzatore_role
    ...
    FOR EACH ROW EXECUTE FUNCTION check_organizzatore_role();

-- Verifica che solo giudici possano essere invitati come giudici
CREATE OR REPLACE FUNCTION check_giudice_role()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM utenti u
...
CREATE TRIGGER trigger_single_team_per_hackathon
    BEFORE INSERT OR UPDATE ON membri_team
    FOR EACH ROW EXECUTE FUNCTION check_single_team_per_hackathon();
...
CREATE OR REPLACE FUNCTION check_registration_before_team()
RETURNS TRIGGER AS $$
DECLARE
    hack_id INTEGER;
BEGIN
    -- Ottieni l'hackathon_id dal team
    SELECT hackathon_id INTO hack_id
    FROM team
    WHERE team_id = NEW.team_id;
...
CREATE OR REPLACE FUNCTION update_registration_team()
RETURNS TRIGGER AS $$
DECLARE
    hack_id INTEGER;
BEGIN
...
CREATE OR REPLACE FUNCTION check_judge_can_vote()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
CREATE OR REPLACE FUNCTION check_team_definitivo_for_vote()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
CREATE OR REPLACE FUNCTION check_hackathon_status_for_vote()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
CREATE OR REPLACE FUNCTION check_team_leader_invite()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
CREATE OR REPLACE FUNCTION update_invite_response_date()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.stato_invito_id = 1 AND NEW.stato_invito_id IN (2, 3) THEN
...
CREATE TRIGGER trigger_invite_response_date
    BEFORE UPDATE ON inviti_team
    FOR EACH ROW EXECUTE FUNCTION update_invite_response_date();

-- Automaticamente unisci al team quando l'invito viene accettato
CREATE OR REPLACE FUNCTION auto_join_team_on_accept()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.stato_invito_id = 2 AND OLD.stato_invito_id = 1 THEN
...
CREATE TRIGGER trigger_auto_join_team
    AFTER UPDATE ON inviti_team
    FOR EACH ROW EXECUTE FUNCTION auto_join_team_on_accept();

-- Verifica che ci sia almeno un leader per team
CREATE OR REPLACE FUNCTION ensure_team_has_leader()
RETURNS TRIGGER AS $$
DECLARE
    leader_count INTEGER;
BEGIN
    IF OLD.ruolo_team = 'LEADER' THEN
        SELECT COUNT(*)
        INTO leader_count
        FROM membri_team
        WHERE team_id = OLD.team_id
...
CREATE TRIGGER trigger_ensure_team_leader_on_update
    BEFORE UPDATE OF ruolo_team ON membri_team
    FOR EACH ROW 
    WHEN (OLD.ruolo_team = 'LEADER' AND NEW.ruolo_team != 'LEADER')
